...
import (
	...
	myStrings "github.com/cloudfly/go/str"
)
...
func RegionOfCity(key string) Name {
	return RegionOfProvince(strings.Split(City(key).EN, "_")[0])
}
...
func genFunc(names []Name) func(string) Name {
	...
	for _, name := range names {
		...
		table[name.CN] = name
		table[name.EN] = name
		table[strings.ToLower(name.EN)] = name
		for _, short := range name.Short {
			table[short] = name
			table[strings.ToLower(short)] = name
		}
	}

	return func(key string) Name {
		x, ok := table[key]
		if ok {
			return x
		}
...
func TrimArea(area string) (string, bool) {
	suffixes := []string{"省", "市", "壮族自治区", "回族自治区", "自治区", "特别行政区"}
	return myStrings.TrimAny(area, suffixes)
}

func TrimCity(city string, defaultCity string) (string, bool) {
	suffixes := []string{"直辖市", "市辖区", "市", "县"}
	newCity, changed := myStrings.TrimAny(city, suffixes)
	if changed && len(newCity) == 0 {
		newCity = defaultCity
	}
	return newCity, changed
}
...
func CitiesInProvince(name string) []Name {
	...
	for _, city := range Cities() {
		if strings.HasPrefix(city.EN, p.EN+"_") {
...
func CityIspFromAlias(alias string) string {
	...
	// dx- 属于节点探测场景的名称规则
	if strings.HasPrefix(alias, "dx-") {
		alias = strings.TrimLeft(alias, "dx-")
		idx := strings.IndexByte(alias, '-')
		province, alias := alias[:idx], alias[idx+1:]
		idx = strings.IndexByte(alias, '-')
		city := alias[:idx]

		City(strings.ToLower(province + "_" + city))
	}
	...
	j := len(alias) - 1
	for ; j > 0; j-- {
		if alias[j] >= '0' && alias[j] <= '9' {
			continue
		}
		break
	}
	alias = strings.ToLower(alias[:j+1])

	if len(alias) < 4 {
		// 通常为 省份缩写+城市缩写+运营商缩写，不能少于 4 个字符
		return "XX"
	}

	isp := "电信" // 默认就电信了
	switch alias[len(alias)-2:] {
	case "ct":
		isp = "电信"
	case "cm":
		isp = "移动"
	...
	var (
		cityCn    = ""
		cityShort = alias[:len(alias)-2]
	)

	//早期节点，通常以省份缩写命名，所以优先尝试获取省份中文名
	cityCn, _ = TrimArea(Province(cityShort).CN)

	if cityCn == "-" {
		// 省份没查到，尝试获取城市中文名
		cityCn, _ = TrimCity(City(cityShort).CN, "-")
	}
	return cityCn + isp
}
