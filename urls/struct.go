...
func Values(v interface{}) (url.Values, error) {
	return ValuesTag(v, "url")
}

// ValuesTag is similar with Values function, but can specify field tag name instead of using `url`
// e.g. Values is equal to ValuesTag(v, "url"), ValuesTag(value, "json") means using encode setting in json tag
func ValuesTag(v any, fieldTag string) (url.Values, error) {
	...
	err := reflectValueFromTag(values, val, "", fieldTag)
	return values, err
}

// reflectValueFromTag populates the values parameter from the struct fields in val.
// Embedded structs are followed recursively (using the rules defined in the
// Values function documentation) breadth-first.
func reflectValueFromTag(values url.Values, val reflect.Value, scope, fieldTag string) error {
	if fieldTag == "" {
		fieldTag = "url"
	}
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if sv.Kind() == reflect.Struct {
			if err := reflectValueFromTag(values, sv, name, fieldTag); err != nil {
				return err
			}
			continue
		}
	...
	for _, f := range embedded {
		if err := reflectValueFromTag(values, f, scope, fieldTag); err != nil {
			return err
		}
	}
